...
for tag in tags:
  ...
  #Remove anything after a space in Ct fields: (some entries in LIS ct field have Ct duplicated (e.g. 21.87  21.87))
...
#----------Add ct=0 to all NEG controls (to prevent errors for libraries with all Ct= NA)--------
#add 0 to the NEG ct field (instead of NA) just so something’s there for every run instead of NA

conditions3 = [
    (df_FastqList2_CleanMetadata_merge10['sample'].astype(str).str.contains('NEG')),    
    (~df_FastqList2_CleanMetadata_merge10['sample'].astype(str).str.contains('NEG'))
]

choices3 = [df_FastqList2_CleanMetadata_merge10['ct'].replace('NA', '0.0'), df_FastqList2_CleanMetadata_merge10['ct']]

df_FastqList2_CleanMetadata_merge10['ct'] = np.select(conditions3, choices3, default= df_FastqList2_CleanMetadata_merge10['ct'])
#print(df_FastqList2_CleanMetadata_merge10['ct'])
